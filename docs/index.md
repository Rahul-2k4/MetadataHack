# MetadataHack Docs Index

This folder tracks the submission package for **Back to the Metadata (Apr 17-26, 2026)**.

## Documents

- [Hackathon Brief](./hackathon-brief.md): official links, rules summary, and judging criteria.
- [Hackathon Rules](./hackathon-rules.md): full rule snapshot and team interpretation.
- [Hackathon Resources](./hackathon-resources.md): setup links, support channels, and official references.
- [Hackathon Schedule](./hackathon-schedule.md): event timeline with UTC and IST conversions.
- [Ideas and Tracks](./ideas-tracks.md): track list and team idea template.
- [Idea Backlog](./idea-backlog.md): candidate ideas and shortlist decisions.
- [Architecture](./architecture.md): architecture template and evolving technical decisions.
- [Demo Script](./demo-script.md): short walkthrough script for judging.
- [Submission Checklist](./submission-checklist.md): pre-submit gate checks.
- [Decision Log](./decision-log.md): key choices and why we made them.

## Working Rule

All docs in this folder should stay hackathon-specific and submission-focused.

## Refresh Procedure

When official pages change, refresh these docs first:

1. `hackathon-rules.md`
2. `hackathon-resources.md`
3. `hackathon-schedule.md`
4. `hackathon-brief.md`
